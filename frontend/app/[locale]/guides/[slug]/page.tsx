...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const post = await getPost(locale, "guides", slug) ?? await getPost("en", "guides", slug);
  if (!post) return {};
  ...
  for (const [loc, locSlug] of Object.entries(post.slugs)) {
    languages[loc] = `${BASE_URL}/${loc}/guides/${locSlug}`;
  }
...
export async function generateStaticParams() {
  ...
  for (const locale of routing.locales) {
    const slugs = getAllPostSlugs(locale, "guides");
    for (const slug of slugs) {
      params.push({ locale, slug });
    }
    if (locale !== "en") {
      const enSlugs = getAllPostSlugs("en", "guides");
    ...
    <>
      ...
      <div style={{ background: "var(--lib-bg)", minHeight: "100vh" }}>
        <ArticleLayout
          ...
          tableOfContentsLabel={tBlog("tableOfContents")}
        />
