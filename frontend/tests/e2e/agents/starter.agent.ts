 ...
 * 검증 항목:
 * - 로그인 성공
 * - 대시보드: "Plan: Starter" 표시
 * - 대시보드: "Manage subscription" 버튼 표시 (유료 플랜)
 * - 계정 페이지: "Starter" 플랜 표시
 * - 변환 페이지: 접근 가능
 */
export function runStarterAgent(config: PlanConfig): void {
  ...
  test('대시보드: Starter 플랜 레이블 표시', async ({ page }) => {
    await expectDashboardPlan(page, config.displayLabel);
  });

  test('대시보드: 구독 관리 버튼 표시 (유료 플랜)', async ({ page }) => {
  ...
  test('계정 페이지: Starter 플랜 표시', async ({ page }) => {
